...
def add_header(response):
    ...
    if request.path.startswith('/admin'):
        ...
        response.headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; font-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com data:; img-src 'self' data: blob:; connect-src 'self'"
    else:
...
def csrf_exempt_admin_routes(response):
    ...
    if request.path.startswith('/admin/closing-costs/') or request.path == '/admin/pmi-rates/update':
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Run the Mortgage Calculator Flask app')
