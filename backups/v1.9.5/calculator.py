...
class MortgageCalculator:
    ...
    def calculate_monthly_payment(self, principal: float, annual_rate: float, years: int) -> float:
        ...
        try:
            self.logger.info(f"Calculating monthly payment: principal={principal}, rate={round(annual_rate, 3)}, years={years}")
            if annual_rate == 0:
    ...
    def calculate_pmi(self, loan_amount: float, home_value: float, loan_type: str, loan_term_months: int = 360) -> float:
        """
        ...
        """
        try:
            self.logger.info(f"Calculating mortgage insurance: loan={loan_amount}, value={home_value}, type={loan_type}")
            ...
            if loan_type == 'conventional':
                ...
                for ltv_range, rate in ltv_ranges.items():
                    ...
                    if len(parts) == 2:
                        ...
                        if min_ltv <= ltv <= max_ltv:
                            ...
                            self.logger.info(f"Selected LTV range for PMI: {ltv_range}%, rate: {round(rate, 3)}%")
                            break
                ...
                if ltv_rate == 0:
                    ...
                    self.logger.info(f"Using highest LTV range: {highest_range[0]}%, rate: {round(highest_range[1], 3)}%")
    ...
    def calculate_mortgage_insurance(self, loan_amount: float, home_value: float, loan_type: str, loan_term_months: int = 360) -> float:
        """
    ...
    def calculate_pmi(self, loan_amount: float, home_value: float, loan_type: str, loan_term_months: int = 360) -> float:
        ...
        return self.calculate_mortgage_insurance(loan_amount, home_value, loan_type, loan_term_months)
    ...
    def calculate_all(self, purchase_price: float, down_payment: float, annual_rate: float, loan_term: int, 
                     annual_tax_rate: float, annual_insurance_rate: float, loan_type: str,
        ...
        try:
            ...
            down_payment_percentage = round((down_payment_amount / purchase_price) * 100, 3)
            ...
            if loan_type.lower() == 'fha':
                monthly_mi = self.calculate_mortgage_insurance(loan_amount, purchase_price, loan_type, loan_term * 12)
            else:
                monthly_mi = self.calculate_mortgage_insurance(base_loan_amount, purchase_price, loan_type, loan_term * 12)
