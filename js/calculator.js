...
Loan Type: ${result.loan_type || 'N/A'}`;
    ...
    formatPurchaseResults(result) {
        ...
        const loanDetails = result.loan_details || {};
        const closingCosts = result.closing_costs || {};
        ...
        return `
            <!-- Monthly Payment Breakdown -->
            <div class="mb-4">
                <h5 class="section-title"><i class="bi bi-calculator"></i> Monthly Payment Breakdown</h5>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <tbody>
                            ...
                            <tr>
                                <td>PMI:</td>
                                <td class="text-end">$${this.formatCurrency(monthly.pmi || 0)}</td>
                            </tr>
                            <tr>
                                <td>HOA Fee:</td>
            ...
            <div class="mb-4">
                <h5 class="section-title"><i class="bi bi-cash-stack"></i> Cash Needed at Closing</h5>

                <!-- Down Payment -->
                <h6 class="section-subtitle">Down Payment</h6>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <tbody>
                            <tr>
                ...
                <!-- Closing Costs -->
                <h6 class="section-subtitle">Closing Costs</h6>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <tbody>
                            ${this.formatClosingCostsRows(closingCosts)}
                        </tbody>
                        <tfoot class="total-row">
                            <tr>
                                <td><strong>Total Closing Costs</strong></td>
                                <td class="text-end"><strong>$${this.formatCurrency(closingCosts.total || 0)}</strong></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <!-- Prepaid Items -->
                <h6 class="section-subtitle">Prepaid Items</h6>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <tbody>
                            ${this.formatPrepaidItemsRows(prepaidItems)}
                        </tbody>
                        <tfoot class="total-row">
                            <tr>
                                <td><strong>Total Prepaid Items</strong></td>
                                <td class="text-end"><strong>$${this.formatCurrency(prepaidItems.total || 0)}</strong></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <!-- Credits -->
                <h6 class="section-subtitle">Credits</h6>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <tbody>
                            ${this.formatCreditsRows(credits)}
                        </tbody>
                        <tfoot class="total-row">
                            <tr>
                                <td><strong>Total Credits</strong></td>
                                <td class="text-end"><strong>$${this.formatCurrency(credits.total || 0)}</strong></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <!-- Total Cash Needed -->
                <h6 class="section-subtitle">Total Cash Needed</h6>
                <div class="table-responsive">
                    <table class="table table-sm">
                        <tbody>
                            <tr class="total-row">
                                <td><strong>Total Cash to Close</strong></td>
                                <td class="text-end"><strong>$${this.formatCurrency(totalCashNeeded)}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <!-- Loan Details -->
            <div class="mb-3">
                <h6 class="section-subtitle"><i class="bi bi-house-door"></i> Loan Details</h6>
                <div class="row g-2 small">
                    <div class="col-6">Loan Amount:</div>
                    <div class="col-6 text-end">$${this.formatCurrency(loanDetails.loan_amount || 0)}</div>

                    <div class="col-6">Interest Rate:</div>
                    <div class="col-6 text-end">${loanDetails.interest_rate || 0}%</div>

                    <div class="col-6">LTV Ratio:</div>
                    <div class="col-6 text-end">${loanDetails.ltv_ratio || 0}%</div>

                    <div class="col-6">Loan Term:</div>
                    <div class="col-6 text-end">${loanDetails.loan_term_years || 30} years</div>
                </div>
            </div>
    ...
    formatClosingCostsRows(closingCosts) {
        const excludeKeys = ['total'];
        return Object.entries(closingCosts)
            ...
            .map(([key, value]) => {
                ...
                return `
                    <tr>
                        <td>${displayName}:</td>
                        <td class="text-end">$${this.formatCurrency(value || 0)}</td>
                    </tr>
                `;
            }).join('');
    }

    formatPrepaidItemsRows(prepaidItems) {
        const excludeKeys = ['total'];
        return Object.entries(prepaidItems)
            .filter(([key]) => !excludeKeys.includes(key))
            .map(([key, value]) => {
                const displayName = this.formatKeyName(key);
                return `
    ...
    formatKeyName(key) {
        // Convert snake_case and camelCase to readable names
        return key
            .replace(/_/g, ' ')
            .replace(/([A-Z])/g, ' $1')
            .split(' ')
    ...
    showLoading() {
        ...
        submitButtons.forEach(btn => {
            btn.disabled = true;
            btn.innerHTML = '<span class="loading-spinner me-2"></span>Calculating...';
    ...
    hideLoading() {
        ...
        if (refinanceBtn) {
            ...
            refinanceBtn.style.pointerEvents = 'auto';
            refinanceBtn.style.opacity = '1';
        }
    }
