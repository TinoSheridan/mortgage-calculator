...
def interactive_calculator():
    ...
    while True:
        ...
        if params['loan_type'].lower() == 'va':
            ...
            va_disability_exempt = True if disability_exempt_input in ('yes', 'y', 'true') else False
        ...
        try:
            ...
            print(f"Base Loan Amount: ${result['loan_details']['purchase_price'] - result['loan_details']['down_payment']:,.2f}")
            
            # FHA loans include upfront MIP in the total loan amount
            if params['loan_type'].lower() == 'fha':
                print(f"Upfront MIP: ${result['loan_details']['loan_amount'] - (result['loan_details']['purchase_price'] - result['loan_details']['down_payment']):,.2f}")
