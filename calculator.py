...
class MortgageCalculator:
    ...
    def calculate_monthly_payment(self, principal: float, annual_rate: float, years: int) -> float:
        ...
        try:
            self.logger.info(f"Calculating monthly payment: principal={principal}, rate={annual_rate}, years={years}")
            if annual_rate == 0:
    ...
    def calculate_pmi(self, loan_amount: float, home_value: float, credit_score: int, loan_type: str, loan_term_months: int = 360) -> float:
        """
        ...
        """
        try:
            self.logger.info(f"Calculating mortgage insurance: loan={loan_amount}, value={home_value}, score={credit_score}, type={loan_type}")
            ...
            if loan_type == 'conventional':
                ...
                self.logger.info(f"Conventional loan PMI calculation: LTV={ltv:.2f}%")
                ...
                for ltv_range, rate in ltv_ranges.items():
                    ...
                    if len(parts) == 2:
                        ...
                        if min_ltv <= ltv <= max_ltv:
                            ...
                            self.logger.info(f"Selected LTV range for PMI: {ltv_range}%, rate: {rate}%")
                            break
                
                if ltv_rate == 0:
                    self.logger.warning(f"No matching LTV range found for {ltv:.1f}%, using default rate")
                    ...
                    self.logger.info(f"Using highest LTV range: {highest_range[0]}%, rate: {highest_range[1]}%")
                ...
                for score_range, adjustment in credit_score_adjustments.items():
                    ...
                    if len(parts) == 2:
                        ...
                        if min_score <= credit_score <= max_score:
                            ...
                            self.logger.info(f"Selected credit score range: {score_range}, adjustment: {adjustment}%")
                            break
                ...
                self.logger.info(f"Final monthly PMI for conventional loan: {rounded_pmi}")
                return rounded_pmi
            ...
            # Process FHA MIP
                ...
                self.logger.info(f"Calculating FHA MIP with loan_term={loan_term_months / 12} years, loan_amount=${loan_amount:,.2f}, ltv={ltv:.2f}%")
                
                # Determine which rate to use based on loan term
                if loan_term_months / 12 > 15:
                    term_category = "long_term"
                    self.logger.info(f"Using long-term FHA MIP rates (>{loan_term_months / 12} years)")
                else:
                    term_category = "short_term"
                    self.logger.info(f"Using short-term FHA MIP rates (≤{loan_term_months / 12} years)")
                
                # Determine which rate to use based on loan amount
                if loan_amount <= standard_loan_limit:
                    amount_category = "standard_amount"
                    self.logger.info(f"Using standard loan amount FHA MIP rates (≤${standard_loan_limit:,.2f})")
                else:
                    amount_category = "high_amount"
                    self.logger.info(f"Using high loan amount FHA MIP rates (>${standard_loan_limit:,.2f})")
                ...
                try:
                    ...
                    annual_mip_rate = pmi_rates.get('annual_mip', {}).get(term_category, {}).get(amount_category, {}).get(ltv_category, 0)
                    if annual_mip_rate == 0:
                        self.logger.warning(f"Could not find specific MIP rate for {term_category}/{amount_category}/{ltv_category}, using default")
                ...
                monthly_mip = (loan_amount * annual_mip_rate) / 12  # Using total loan amount including upfront MIP
            ...
            # Process VA guarantee fee
                ...
                self.logger.info(f"Final monthly guarantee fee for USDA loan: ${rounded_fee:.2f}")
                return rounded_fee
            
            # Other loan types
            self.logger.info(f"No monthly mortgage insurance for loan type: {loan_type}")
            return 0.0
    ...
    def calculate_mortgage_insurance(self, loan_amount: float, home_value: float, credit_score: int, loan_type: str, loan_term_months: int = 360) -> float:
        """
        ...
        Parameters:
            ...
            credit_score (int): Credit score of the borrower
        ...
        """
        try:
            self.logger.info(f"Calculating mortgage insurance: loan_amount=${loan_amount:,.0f}, "
    ...
    def calculate_pmi(self, loan_amount: float, home_value: float, credit_score: int, loan_type: str, loan_term_months: int = 360) -> float:
        ...
        return self.calculate_mortgage_insurance(loan_amount, home_value, credit_score, loan_type, loan_term_months)

    def calculate_va_funding_fee(self, loan_amount: float, down_payment_percentage: float, 
                                service_type: str, loan_usage: str, disability_exempt: bool = False) -> float:
        """
        ...
        if disability_exempt:
            self.logger.info("VA funding fee exemption applied due to disability status")
            return 0.0
        
        self.logger.info(f"Calculating VA funding fee with loan_amount=${loan_amount:,.2f}, "
        ...
        try:
            ...
            if service_type not in ['active', 'reserves']:
                self.logger.warning(f"Invalid service type: {service_type}. Defaulting to 'active'")
                service_type = 'active'
            
            # Validate loan usage
            if loan_usage not in ['first', 'subsequent']:
                self.logger.warning(f"Invalid loan usage: {loan_usage}. Defaulting to 'first'")
                loan_usage = 'first'
            ...
            if not service_rates:
                self.logger.error(f"No funding fee rates found for service type: {service_type}")
            ...
            if not bracket_rates:
                self.logger.error(f"No funding fee rates found for down payment bracket: {dp_bracket}")
            ...
            if fee_rate is None:
                self.logger.error(f"No funding fee rate found for loan usage: {loan_usage} in bracket {dp_bracket}")
                fee_rate = bracket_rates.get('first')
                self.logger.info(f"Falling back to 'first' fee rate")
                
            self.logger.info(f"Fee rate lookup for {service_type}, {dp_bracket}, {loan_usage}: {fee_rate}")
            
            # Calculate fee
            fee = loan_amount * (fee_rate / 100)
            self.logger.info(f"Calculated VA funding fee: ${fee:.2f} (rate: {fee_rate}%, loan_amount: ${loan_amount:.2f})")
            
            return fee
            
        except Exception as e:
            self.logger.error(f"Error calculating VA funding fee: {e}")
            self.logger.error(f"Parameters: loan_amount=${loan_amount}, down_payment={down_payment_percentage}%, "
    ...
    def calculate_financed_fees(self, loan_amount: float, home_value: float, loan_type: str, 
                              down_payment_percentage: float, va_service_type: str = None, 
                              va_usage: str = None, va_disability_exempt: bool = False) -> float:
        """
        ...
        """
        try:
            self.logger.info(f"Calculating financed fees: loan_type={loan_type}, loan_amount={loan_amount}")
            loan_type = loan_type.lower()
            ...
            if loan_type == 'conventional':
                ...
                self.logger.info(f"FHA upfront MIP: ${total_financed_fees:.2f} (rate: {upfront_mip_rate * 100}%)")
                
            elif loan_type == 'va':
                # Calculate VA funding fee
                self.logger.info(f"Processing VA loan with parameters: va_service_type={va_service_type}, "
                ...
                if not va_service_type:
                    ...
                    self.logger.warning(f"VA service_type not provided, using default: {va_service_type}")
                
                if not va_usage:
                    va_usage = 'first'
                    self.logger.warning(f"VA loan_usage not provided, using default: {va_usage}")
                
                if va_disability_exempt is None:
                    va_disability_exempt = False
                    self.logger.warning("VA disability_exempt not provided, using default: False")
                
                # Special handling for subsequent use to ensure proper fee calculation
                if va_usage == 'subsequent':
                    self.logger.info("VA loan is marked as 'subsequent use', ensuring proper funding fee calculation")
                ...
                try:
                    ...
                    self.logger.info(f"Successfully calculated VA funding fee: ${total_financed_fees:.2f} for {va_usage} use")
                except Exception as e:
                    self.logger.error(f"Error in VA funding fee calculation: {e}")
                    # Use a default if calculation fails
                    if not va_disability_exempt:
                        default_fee_rate = 2.3  # Default to 2.3% if unable to calculate precisely
                        total_financed_fees = round((loan_amount * default_fee_rate) / 100, 2)
                        self.logger.warning(f"Using default VA funding fee rate: {default_fee_rate}%, fee=${total_financed_fees:.2f}")
                    else:
                        total_financed_fees = 0.0
                        self.logger.info("No VA funding fee due to disability exemption")
                ...
                self.logger.info(f"USDA upfront guarantee fee: ${total_financed_fees:.2f} (rate: {upfront_fee_rate * 100}%)")
    ...
    def calculate_all(self, purchase_price: float, down_payment: float, annual_rate: float, loan_term: int, 
                     annual_tax_rate: float, annual_insurance_rate: float, credit_score: int, loan_type: str,
                     hoa_fee: float = 0.0, seller_credit: float = 0.0, lender_credit: float = 0.0, 
                     discount_points: float = 0.0, va_service_type: str = None, va_usage: str = None,
                     va_disability_exempt: bool = None, closing_date = None, **kwargs) -> Dict[str, Any]:
        """
        ...
        """
        try:
            self.logger.info(f"Starting mortgage calculation: price=${purchase_price:,.0f}, down_payment=${down_payment:,.0f}, "
                           f"rate={annual_rate}%, term={loan_term} years, loan_type={loan_type}")
            self.logger.info(f"Received closing_date: {closing_date} (type: {type(closing_date).__name__ if closing_date else None})")
            
            # Convert inputs to proper formats
            loan_type = loan_type.lower()
            down_payment_amount = float(down_payment)
            down_payment_percentage = (down_payment_amount / purchase_price) * 100
            
            # Calculate base loan amount (before any financed fees)
            base_loan_amount = purchase_price - down_payment_amount
            base_ltv = (base_loan_amount / purchase_price) * 100
            
            self.logger.info(f"Base calculations: down_payment=${down_payment_amount:,.2f}, "
                           f"down_payment_percentage={down_payment_percentage:.1f}%, loan_type={loan_type}")
            
            # VA loan specific handling
            if loan_type == 'va':
                self.logger.info(f"VA loan parameters: service_type={va_service_type}, usage={va_usage}, "
            ...
            if financed_fees > 0:
                ...
                self.logger.info(f"Added {loan_type} {fee_type} to loan amount: ${financed_fees:.2f}")
                self.logger.info(f"Final loan amount (after adding {fee_type}): ${loan_amount:,.2f}")
            ...
            if credit_score < loan_config['min_credit_score']:
                raise ValueError(f"Credit score {credit_score} below minimum {loan_config['min_credit_score']} for {loan_type} loans")
            
            # Calculate monthly principal and interest
            monthly_pi = self.calculate_monthly_payment(loan_amount, annual_rate, loan_term)
            
            # Calculate monthly mortgage insurance (PMI/MIP/guarantee fee)
            if loan_type.lower() == 'fha':
                monthly_mi = self.calculate_mortgage_insurance(loan_amount, purchase_price, credit_score, loan_type, loan_term * 12)
            else:
                monthly_mi = self.calculate_mortgage_insurance(base_loan_amount, purchase_price, credit_score, loan_type, loan_term * 12)
            ...
            monthly_payment = monthly_pi + monthly_mi + monthly_tax + monthly_insurance + hoa_fee
            self.logger.info(f"Monthly payment breakdown: P&I=${monthly_pi:.2f}, MI=${monthly_mi:.2f}, "
            ...
            if discount_points > 0:
                ...
                self.logger.info(f"Added discount points to closing costs: points={discount_points}%, cost=${points_cost:,.2f}")
            
            # Calculate prepaid items
            prepaid_items = self.calculate_prepaid_items(
                loan_amount, annual_tax_rate, annual_insurance_rate, annual_rate, closing_date
            )
            
            # Apply credits
            total_credits = seller_credit + lender_credit
            net_closing_costs = closing_costs['total'] + prepaid_items['total'] - total_credits
            self.logger.info(f"Credits and final costs: credits=${total_credits:,.2f}, net_closing_costs=${net_closing_costs:,.2f}")
            ...
            results = {
                'loan_details': {
                    ...
                    'ltv': round(base_ltv, 2)  # Use base LTV for display
                },
                'monthly_payment': {
    ...
    def calculate_closing_costs(self, purchase_price: float, loan_amount: float) -> Dict[str, float]:
        """Calculate itemized closing costs based on configuration."""
        try:
            self.logger.info(f"Calculating closing costs: price={purchase_price}, loan={loan_amount}")
            closing_costs = {}
            total = 0.0
            ...
            for item_name, item_config in self.config['closing_costs'].items():
                ...
                if cost_type == 'fixed':
                    ...
                    if base == 'purchase_price':
                        ...
                        self.logger.warning(f"Unknown calculation base for {item_name}: {base}")
                        continue
                else:
                    self.logger.warning(f"Unknown cost type for {item_name}: {cost_type}")
                    continue
    ...
    def calculate_prepaid_items(self, loan_amount: float, annual_tax_rate: float, 
                               annual_insurance_rate: float, annual_interest_rate: float,
                               closing_date = None) -> Dict[str, float]:
        """Calculate prepaid items (taxes, insurance, interest)."""
        try:
            self.logger.info(f"Calculating prepaid items with loan={loan_amount}, tax_rate={annual_tax_rate}, insurance_rate={annual_insurance_rate}, interest_rate={annual_interest_rate}")
            self.logger.info(f"Closing date provided: {closing_date} (type: {type(closing_date).__name__ if closing_date else None})")
            ...
            self.logger.info(f"Daily interest calculation: {loan_amount} * {annual_interest_rate}% / 360 = ${daily_interest:.2f}/day")
            ...
            if closing_date:
                try:
                    ...
                    self.logger.info(f"Calculated {days_of_interest} days from closing date {closing_date} to end of month {last_date_of_month}")
                except Exception as e:
            ...
            self.logger.info(f"Prepaid interest calculation: ${daily_interest:.2f}/day × {days_of_interest} days = ${prepaid_interest:.2f}")
            ...
            self.logger.info(f"Property tax calculations: monthly=${monthly_tax:.2f}, prepaid=${prepaid['prepaid_tax']:.2f}, escrow=${prepaid['tax_escrow']:.2f}")
            
            # 3. Calculate prepaid homeowner's insurance
            monthly_insurance = (loan_amount * annual_insurance_rate / 100) / 12
            prepaid['prepaid_insurance'] = round(monthly_insurance * config['months_insurance_prepaid'], 2)
            prepaid['insurance_escrow'] = round(monthly_insurance * config['months_insurance_escrow'], 2)
            self.logger.info(f"Insurance calculations: monthly=${monthly_insurance:.2f}, prepaid=${prepaid['prepaid_insurance']:.2f}, escrow=${prepaid['insurance_escrow']:.2f}")
    ...
    def generate_amortization_data(self, principal: float, annual_rate: float, years: int) -> list:
        ...
        try:
            self.logger.info(f"Generating amortization data: principal={principal}, rate={annual_rate}, years={years}")
