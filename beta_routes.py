...
def submit_feedback():
    ...
    if not current_app.config.get('BETA_ENABLED', False):
        return jsonify({'status': 'error', 'message': 'Beta testing is not enabled'}), 404
    ...
    try:
        ...
        current_app.logger.info(f"Beta feedback received: {feedback_data.get('feedback_type')} from {feedback_data.get('user_email', 'anonymous')}")
